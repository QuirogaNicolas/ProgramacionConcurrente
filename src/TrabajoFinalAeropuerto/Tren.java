...
public class Tren {
    ...
    public void abordar() throws InterruptedException {
        ...
        try {
            // Una vez arriba del tren van a esperar a la partida del mismo
    ...
    public void bajar(int terminalPasajero) {
        ...
        System.out.println(Thread.currentThread().getName() + " yo me bajé en la parada " + terminalActual
                + " y me correspondía " + terminalPasajero);
        capacidad++;
        lock.unlock();
    }
    ...
    public void volvimos() {
        // El maquinista avisa que ya volvió a los pasajeros que están esperando para
        // subir al tren
        lock.lock();
        terminalActual = 0;
        meSubo.signalAll();
    ...
    public void partir() throws InterruptedException {
        // El maquinista espera a que se llene el tren
        try {
            barreraParaSalida.await();
            System.out.println(Thread.currentThread().getName() + " dice: se va el tren!");
        } catch (BrokenBarrierException e) {
            if (cerrado) {
                System.out.println(Thread.currentThread().getName() + "dice: la estación cerró!");
                Thread.currentThread().interrupt();
                throw new InterruptedException();
            }
        } catch (InterruptedException e) {
    ...
    public void cerrar() {
        // Desde el main se indica que cerró la estación
        cerrado = true;
    }
