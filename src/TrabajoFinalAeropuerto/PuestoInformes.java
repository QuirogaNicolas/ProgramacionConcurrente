    ...
    public synchronized Object[] asignarPuestoAtencion(String vuelo) {
        while (!desocupado) {
            try {
                ...
                System.out.println(
                        Thread.currentThread().getName() + " interrumpido en la espera del puesto de informes");
            }
        }
        desocupado = false;
        // Devuelve el puesto de atención correpondiente según las primeras 3 letras del
        // código de vuelo
        return mapaAerolineas.get(vuelo.substring(0, 3));
    }

    public synchronized void liberarPuesto() {
        // Da el lugar al próximo pasajero
        desocupado = true;
        notify();
    }
